...
const Trang = ({ posts: initialPosts, posters, allPosts: initialNumPosts }) => {
  ...
  const handleLoadMore = async () => {
    setWaiting(true)
    const newPosts = await getPostByType("Điểm Đến", num, where);
  ...
  useEffect(async () => {
    if (where) {
      const newPosts = await getPostByType("Điểm Đến", 1, mappingWhere[where]);
      setPosts(newPosts);
      const newNumPosts = await countPostsByType("Điểm Đến", mappingWhere[where]);
...
Trang.getInitialProps = async (ctx) => {
  const posts = (await getPostByType("Điểm Đến", 1)) || [];
  const posters = await getPosters();
  const allPosts = await countPostsByType("Điểm Đến");
  return {
