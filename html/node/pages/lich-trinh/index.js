...
const Trang = ({ posts: initialPosts, posters, allPosts: initialNumPosts }) => {
  ...
  const handleLoadMore = async () => {
    setWaiting(true)
    const newPosts = await getPostByType("Lịch Trình", num, where);
  ...
  useEffect(async () => {
    if (where) {
      const newPosts = await getPostByType("Lịch Trình", 1, mappingWhere[where]);
      setPosts(newPosts);
      const newNumPosts = await countPostsByType("Lịch Trình", mappingWhere[where]);
...
Trang.getInitialProps = async (ctx) => {
  const posts = (await getPostByType("Lịch Trình", 1)) || [];
  const posters = await getPosters();
  const allPosts = await countPostsByType("Lịch Trình");
  return {
